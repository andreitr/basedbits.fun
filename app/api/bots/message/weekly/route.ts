    ...
    const results = await Promise.allSettled(
      filteredUsers.map(async (user) => {
        try {
          ...
          const message = `Another week, another airdrop. You have earned yourself another ${messageCreated.bounty} BBITS. This claim link expires in 7 days, so don’t sleep on it https://www.basedbits.fun/users/${user.address}?message=${messageCreated.rand_hash}`;
