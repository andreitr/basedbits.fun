...
export interface GhoulsDrawings {
  ...
  topPrize: bigint;
  // When the current drawing closes
  drawingTime: bigint;
...
export const readGhoulsDrawings = async (
  ...
  const candidates = new Set<bigint>(history.map(([, drawingId]) => drawingId));
  if (currentDrawingId > BigInt(0)) {
    candidates.add(currentDrawingId - BigInt(1));
  }
  const settled = Array.from(candidates).filter((id) => id < currentDrawingId);
